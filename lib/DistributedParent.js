...
class DistributedParent extends Thread {
	...
	constructor(threadInfo, options={}, data=null) {
		...
		this.queue = new Proxy({}, {
			get: (target, key, receiver) => data => {
				this.queuedFunctions++;
				const promise = this._subThreads[this.threadSelect].queue['_callThreadFunction'](key, data, 'queue')
		...
		return new Proxy(this, {
			get: (target, key, receiver) => {
				if (target[key] == undefined && key != 'then') {	
					return data => {
						// this.working = true;
						const promise = this._subThreads[this.threadSelect]['_callThreadFunction'](key, data, 'call')
