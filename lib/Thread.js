...
class Thread {
	...
	_getThreadReferenceData = async threadFile => {
		if (Thread.spawnedThreads[threadFile] != undefined) return await Thread.spawnedThreads[threadFile][0]._callThreadFunction('_getThreadReferenceData')
		if (isMainThread && Thread.spawnedThreads[threadFile] == undefined) throw new Error(`Thread ${threadFile} has not been spawned!`)
