...
class Thread {
	...
	constructor(messagePort, workerData) {
		...
		this._exports = {}
		Object.defineProperty(module.parent, 'exports', {
			get: () => this._exports,
	...
	_getThreadReferenceData = async threadName => {
		if (isMainThread) {
			...
			return await spawnedThreads[threadName]._callThreadFunction('_getThreadReferenceData')
		}
	...
	_functionHandler = messagePort => async message => {
		...
		if (typeof this._exports[message.func] != 'function') messagePort.postMessage({ 
			type: 'reject',
			data: new Error(`Cannot run function. Function ${JSON.stringify(message.func)} is ${typeof this._exports[message.func]}.`),
			promiseKey: message.promiseKey
		})
		else {
			this.working = true
			await this._exports[message.func](message.data)
			.then(
