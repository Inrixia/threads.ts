...
class Thread {
	...
	_functionHandler = messagePort => async message => {
		...
		let funcToExec;
		if (module.parent.exports[message.func] != undefined) funcToExec = module.parent.exports[message.func]
		else if (this._internalFunctions[message.func] != undefined) funcToExec = this._internalFunctions[message.func]
		if (typeof funcToExec != 'function') messagePort.postMessage({ 
			type: 'reject',
			data: new Error(`Cannot run function in thread. Function ${JSON.stringify(message.func)} is ${typeof funcToExec}.`),
...
class DistributedParent {
	...
	constructor(threadInfo, options={}) {
		...
		// Proxy this and the queue so any function calls are translated to thread calls
		this.queue = new Proxy({}, {
			get: (target, key, receiver) => (data, promiseKey) => this._loadBalance('_callThreadFunction', key, data, promiseKey, 'queue')
		})
		return new Proxy(this, {
			get: (target, key, receiver) => {
				if (target[key] == undefined && key != 'then') {	
	...
	_messageHandler = messagePort => async message => {
		...
		switch (message.type) {
			case 'function':
				this._loadBalance(message.func, message.data)
				.then(
					data => messagePort.postMessage({ 
						type: 'resolve',
						data,
			...
			case 'reject':
				...
				break;
			case 'queue':
				this._loadBalanceQueue(message.func, message.data)
				.then(
					data => messagePort.postMessage({ 
						type: 'resolve',
						data,
	...
	/**
	 * Loadbalances queues function calls across threads.
	 * @param {string} func Name of function to call.
	 * @param {*} ...args Arguments to pass to function.
	 * @return {Promise}
	 */
	_loadBalanceQueue = (func, ...args) => this.threads[this.threadSelect].queue[func](...args)
	...
	get queuedFunctions() { 
		return this.threads.map(thread => thread.queuedFunctions)
	}
