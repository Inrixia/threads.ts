...
class Thread {
	...
	_getThreadReferenceData = async threadFile => {
		if (isMainThread) {
			if (Thread.spawnedThreads[threadFile] == undefined) Thread.spawnedThreads[threadFile] = [ new Parent(threadFile) ]
			// if (Thread.spawnedThreads[threadFile][0].constructor == DistributedParent) return await Thread.spawnedThreads[threadFile][0]._getThreadReferenceData()
			else return await Thread.spawnedThreads[threadFile][0]._callThreadFunction('_getThreadReferenceData')
		}
