...
let spawnedThreads = {}
const addThread = (threadName, thread) => {
	if (spawnedThreads[threadName] == undefined) spawnedThreads[threadName] = [ thread ]
	else spawnedThreads[threadName].push(thread)
}
...
class Thread {
	...
	_getThreadReferenceData = async threadFile => {
		if (isMainThread) {
			...
			if (spawnedThreads[threadFile][0].constructor == DistributedParent) return await spawnedThreads[threadFile][0]._getThreadReferenceData()
			else return await spawnedThreads[threadFile][0]._callThreadFunction('_getThreadReferenceData')
		}
	 ...
	 */
	_messageHandler = messagePort => message => {
	...
	_functionHandler = messagePort => async message => {
		...
		if (typeof theFunction != 'function') {
			throw new Error(`Cannot run function in thread. Function ${JSON.stringify(message.func)} is ${typeof theFunction}.`)
		} else {
			let funcToExec
...
class Parent extends Thread {
	...
	constructor(threadInfo, options={}, data=null) {
		...
		super(
			new Worker(threadInfo, { workerData: { sharedArrayBuffer: options.sharedArrayBuffer, data }, eval: options.eval }),
...
module.exports = new Proxy({ Child: Thread, Parent, DistributedParent, spawnedThreads }, {
	get: (target, property, receiver) => {
		if (property === 'Child') {
			if (workerData == null) {
				const nonChildErr = new Error('This module was not a spawned child thread! Only native node module functionality is avalible.')
				return new Proxy({}, {
					get: () => { throw nonChildErr },
					set: () => { throw nonChildErr }
				})
			}
